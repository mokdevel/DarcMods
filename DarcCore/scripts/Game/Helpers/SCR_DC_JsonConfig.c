...
//------------------------------------------------------------------------------------------------
class SCR_DC_JsonConfig : JsonApiStruct
{
...
//		m_fileName = "$profile:/" + fileName;
		...
		if(!success)
		{
			SCR_DC_Log.Add("[SCR_DC_JsonConfig] Config file load failed or not found (" + m_fileName + "). Using and saving a default config.", LogLevel.ERROR);
			return null;
		}

		SCR_DC_Log.Add("[SCR_DC_JsonConfig] Loading configuration from file: " + m_fileName, LogLevel.NORMAL);        							
				
		loadContext.ReadValue("version", version);
		loadContext.ReadValue("author", author);
	...
	SCR_JsonSaveContext SaveConfigOpen(string fileName)
	{
		...
		saveContext.WriteValue("version", version);
		saveContext.WriteValue("author", author);
