...
//		SDRC_Log.Add("[SDRC_RoadHelper:CreateRoute] Route pts created: " + routePts.Count(), LogLevel.SPAM);
			...
			for (int i = 0; i < 20; i++)
			{
				...
				SDRC_DebugHelper.AddDebugPos(ptPos, Color.CYAN, 3, "ROADTEST", 2);
			}
		#endif		
	}
	...
	*/	
	...
	static void SplitRoute(out array<vector>route)
	{
		...
		float distanceToRoad = SDRC_Misc.GetWorldSize() / 27;
	...
	*/	
	static void CombineRoadPts(out array<vector> roadPts1, array<vector> roadPts2)
	{
		...
		{	
			...
			SDRC_Log.Add("[SDRC_RoadHelper] Road to be mirrored: " + j, LogLevel.DEBUG);			
		}
		else
		{
...
*/		
		...
		SDRC_DebugHelper.AddDebugPos(pos, Color.PINK, 2, "ROADTEST", 5);				
		
		SDRC_RoadPos roadPos;
		roadPos = FindClosestRoadposToPos(pos);
		SDRC_DebugHelper.AddDebugPos(roadPos.posOnRoad, Color.YELLOW, 3, "ROADTEST", 5);
