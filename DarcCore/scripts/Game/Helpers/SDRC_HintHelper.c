...
{	
	...
	*/
	static void ShowHint(string title, string msg, int timeOut, SDRC_EMissionIcon icon = -1, SDRC_EHintPosition position = SDRC_EHintPosition.UP_LEFT)
	{	
		SDRC_RplHintComp hintManagerComponent = SDRC_RplHintComp.GetInstance();
		if (hintManagerComponent)
		{
			hintManagerComponent.ShowGlobalHint(title, msg, timeOut, icon, position);
		}
		else
		{
