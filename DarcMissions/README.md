...
All items under each mods ```/Weapons/Rifles``` will be collected. The list could be very long but below is a section.
```
...
```
The ```include``` filter matches the list so everything is included. The ```exclude``` filter picks out anything with "_Base", the SVD's and the specific M16A2 with suppressor. A grenade was specified in the itemList so it is included. The resulting list will be: 
```
