...
/*		vector rotation = vector.Direction(m_vPosOrigin, GetPos());		
	...
	{	
		...
		for (int i = 0; i < 10; i++)
		{
			pos = SDRC_Math.GetCoordinatesOnCircle(pos, SDRC_Misc.RandomInt(150, 200), SDRC_Misc.RandomInt(0, 360));
...
class SDRC_ChopperConfig : SDRC_MissionConfig
{
	...
	{
		SDRC_Misc.GetFileList(missionFiles, SDRC_Conf.subDirPath, ".json", "dc_missionConfig_Chopper_", true);
